...
Three layers of test coverage are planned:

- **Verification matrix** in `object_store_intent.rs`: positive and negative
  tests for `put_opts`, `put_multipart_opts`, `get_opts`, `get_opts` with retry
  tag, range read, head-via-get-opts, head-convenience-drops, delete (cannot
  carry), list (cannot carry); two tests documenting the upstream `BufWriter`
  bug (small payload drops, large payload carries) as regression catches for
  upstream #735; one focused tablestore test constructing a `TableStore` with
  capacity = 1 KB to force multipart and assert intent reaches the wrapper
  through SlateDB's `new_intent_tagged_bufwriter` helper; three end-to-end tests
  driving a real `Db` (WAL flush tagged, memtable flush emits put, CRC mismatch
  emits retry tag).
- **Cache wrapper integration** in `cached_object_store::object_store::tests`:
  construction, stacking, and preservation of intent/extensions through internal
  cache miss fetches. Follow-up policy changes should add focused tests for
  per-intent write admission and read routing when they land.
- **Cross-crate test** in `slatedb-txn-obj`: proves constructor-time
  `Extensions` reach the underlying store on every internal protocol write,
  protocol read, and boundary write, for both single-payload and split-payload
  constructors.

Existing tests that depend on the pre-protocol cache behavior
(`db::tests::test_get_with_object_store_cache_put_caching_enabled`, the
`test_get_with_object_store_cache_metrics` and
`test_db_records_remote_object_store_reads_but_not_cache_hits` tests, the
`should_populate_disk_cache_on_read` test for the reader) will be updated to
reflect the new construction pattern while preserving the current admission
behavior.
Config round-trip tests that probed nested loading via
`object_store_cache_options.root_folder` will move to a different nested field
once the option leaves `Settings`.
