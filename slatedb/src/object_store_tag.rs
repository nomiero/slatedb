//! The per-call tag SlateDB attaches to the object store calls its
//! [`TableStore`](crate::tablestore::TableStore) issues for an SST.
//!
//! The tag travels in [`object_store::Extensions`] on every `GetOptions`,
...
//! is the reader. Keeping the types here (rather than in `tablestore`) lets the
//! bundled object store cache classify calls without depending on the much
//! larger `tablestore` module. See RFC 0027.
...
/// whole tag with one lookup to classify the call by source, SST type, and
/// retry state.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
