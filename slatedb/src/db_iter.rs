...
/// An iterator that evaluates sources sequentially by recency without
/// merging.
///
/// Sources are ordered from most recent to least recent: write batch, active
/// memtable, immutable memtables, L0 SSTs, then sorted runs. Unlike
/// [`DbIterator`], this iterator:
...
/// - Lazily initializes each source iterator only when it becomes the
///   current source, so reading only from the most recent source avoids I/O
///   for older sources entirely.
///
/// Useful for prefix scans where the caller wants the most recent data first
/// (e.g. "get latest N items for sensor X") and can interpret tombstones and
/// merge operands themselves.
pub struct RecencyPrefixIterator {
    /// Iterators ordered from most recent to least recent. Each iterator has
    /// already been wrapped with the sequence-number filter.
