...
By default, writes go straight to the upstream object store and do not populate the object-store cache. You can admit freshly written SSTs to the local cache by source: set [`cache_on_flush`](https://docs.rs/slatedb/latest/slatedb/config/struct.ObjectStoreCacheOptions.html#structfield.cache_on_flush) to `true` to store SSTs produced by memtable flushes, and [`cache_on_compaction`](https://docs.rs/slatedb/latest/slatedb/config/struct.ObjectStoreCacheOptions.html#structfield.cache_on_compaction) to store compaction outputs. Both help when readers are likely to touch freshly written SSTs soon afterward. The WAL is never cached (its writes and reads both bypass the cache), since it is short-lived and read only during replay, and compaction input reads bypass the cache so one-shot scans do not evict hotter entries.
